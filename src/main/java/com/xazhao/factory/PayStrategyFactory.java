package com.xazhao.factory;

import cn.hutool.core.util.ObjectUtil;
import com.xazhao.enums.PayStrategyEnum;
import com.xazhao.exception.ServiceException;
import com.xazhao.strategy.PayStrategy;
import com.xazhao.strategy.impl.AlipayServiceImpl;
import com.xazhao.strategy.impl.WeChatServiceImpl;
import com.xazhao.utils.SpringUtils;
...
public class PayStrategyFactory implements ApplicationListener<ContextRefreshedEvent> {
    ...
    /**
     * 弃用<br/>
     * 第三种方式获取所有的支付策略：定义一个策略的枚举类{@link com.xazhao.enums.PayStrategyEnum}
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        for (PayStrategyEnum e : PayStrategyEnum.values()) {
            ...
            PayStrategy payStrategy = (PayStrategy) SpringUtils.getBean(e.getClassName());
     ...
     * 例如：{@link com.xazhao.strategy.impl.UnionPayServiceImpl}在这个策略实现类中使用{@link @Component("UnionPay")}声明一个Bean
    ...
    public PayStrategy getPayStrategy(String payType) {
        ...
        if (!ObjectUtil.isNotEmpty(payStrategy)) {
            // 未查询到具体策略
            throw new ServiceException("获取指定支付平台出现异常，请检查具体实现！");
        }
