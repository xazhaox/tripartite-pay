...
import com.xazhao.exception.PayException;
...
public class AlipayServiceImpl implements PayStrategy {
    ...
    /**
     * AlipayClient相关信息（初始化）已经在Spring初始化的时候船创建完成{@link AlipayClientConfig#alipayClient()}，可直接注入使用<p/>
     * 若没有在Spring初始化时创建，需要在调用时在自行创建
     *
     * @see AlipayClientConfig#alipayClient()
     */
    @Resource
    ...
    public String tradePagePay(Pay pay) {
        ...
        try {
            // 生成订单
            log.info("{} 支付，模拟生成订单号，订单号为：{}.", pay.getPayType(), pay.getProductId());
            ...
            if (response.isSuccess()) {
                ...
                throw new PayException(pay.getPayType() + " 调用统一收单下单并支付页面接口出现异常.");
            }
        } catch (AlipayApiException e) {
            e.printStackTrace();
            throw new PayException(pay.getPayType() + " 支付出现异常.");
        }
    }
}
